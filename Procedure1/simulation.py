...
class procedure1:
    ...
    def __init__(self, MachineNumber = 5):
        ...
        production_lines = 8#10000 # production lines
        n = 1000  # slot of time
        simulation_num = 10000
        process_num = 8#10

        for i in range(process_num):
            Process(target=self.calculation, args=( i, int(production_lines/process_num), n, simulation_num)).start()
    ...
    def calculation(self, process_num, production_lines, n, simulation_num):
        ...
        for m in range(production_lines):
            ...
            for i in range(n):
                ...
                PR[m][i] = PR[m][i]/production_lines
                CR[m][i] = CR[m][i]/production_lines
                for k in range(self.MachineNumber-1):
                        WIP[m][i][k] = WIP[m][i][k]/production_lines
                        ST[m][i][k] = ST[m][i][k]/production_lines
                        BL[m][i][k] = BL[m][i][k]/production_lines
                # print(m,i)
